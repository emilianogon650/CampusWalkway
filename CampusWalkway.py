...
# From represents the first location
# To represents the second location
walkways = [
...
# Add edges with custom formula for weight = (steps + time) / 2
...
# This line prints all the edges with their weights in the terminal so it is clear to read as graph can become messy
print("Campus Walkways and Weights:")
for u, v, data in G.edges(data=True):
    print(f"{u} <--> {v} : weight = {data['weight']}")

# Example: Find the shortest path between two real nodes in your data
source = "Library"
target = "Soda Center"
...
shortest_path = nx.dijkstra_path(G, source="Recreation Center", target="Credit Union Pavilion", weight="weight")
print("Shortest path from Recreation Center to Credit Union Pavilion:")
print(" -> ".join(shortest_path))

# Bellman-Ford: shortest path from Recreation Center to Credit Union Pavilion
...
# Floyd-Warshall: all-pairs shortest paths (returns a dict of dicts)
fw_distances = nx.floyd_warshall(G, weight="weight")

# Get distance between Recreation Center and Credit Union Pavilion
fw_distance = fw_distances["Sichel Hall"]["Dante Hall"]

print("\nFloyd-Warshall distance from Sichell Hall to Dante Hall:")
...
rounded_labels = {k: f"{v:.1f}" for k, v in edge_labels.items()}  # Round to 1 decimal
